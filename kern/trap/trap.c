...
struct  Env* last_fauled_env = NULL;
void fault_handler(struct Trapframe *tf)
{
	...
	if (last_fault_va == fault_va && last_fauled_env == curenv)
	{
		num_repeated_fault++ ;
	...
	else
	{
		...
		if (userTrap)
		{
			...
			bool kill = isKernal || isReadOnly;
			kill&=isPresent;
			kill|=!isMarked && inHeap;
			if (kill)
